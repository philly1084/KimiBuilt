...
Failed or cancelled provider jobs skip automatic result-file collection because that gateway operation deletes the temporary handoff even when its manifest is incomplete. Partial outputs remain subject to gateway retention and require recovery before expiry; already shared artifacts remain accessible.
