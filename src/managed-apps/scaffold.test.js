...
describe('managed app scaffold', () => {
    test('generates a BuildKit-based Gitea workflow for the external control plane', () => {
        ...
        expect(workflow.content).not.toContain('KIMIBUILT_BUILD_EVENTS_SECRET');
        expect(workflow.content).not.toContain('Notify KimiBuilt on success');
        expect(workflow.content).not.toContain('Notify KimiBuilt on failure');
        expect(workflow.content).not.toContain('post_json() {');
        expect(workflow.content).not.toContain('--post-data="$payload"');
        expect(workflow.content).not.toContain('KIMIBUILT_BUILD_EVENTS_INSECURE');
        expect(workflow.content).not.toContain('--no-check-certificate');
