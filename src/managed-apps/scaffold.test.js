...
describe('managed app scaffold', () => {
    test('generates a BuildKit-based Gitea workflow for the external control plane', () => {
        ...
        expect(workflow.content).toContain('BUILD_EVENTS_URL: https://kimibuilt.demoserver2.buzz/api/integrations/gitea/build-events');
        expect(workflow.content).toContain('- name: Notify KimiBuilt');
        expect(workflow.content).toContain('if: always()');
