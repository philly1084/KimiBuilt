...
function buildRemoteCliInstructions({
  ...
  return [
    ...
    'For new apps without a remote, create or use a repository under the configured GitLab group when the configured provider token is available to the backend or remote workbench. If the remote CLI tools cannot create the repo directly, commit locally, report the exact missing credential/API capability, and leave the workspace ready to attach to GitLab instead of editing live cluster state only.',
    'If GitLab is not configured or not reachable, deliberately fall back to a local git repo plus the direct BuildKit/kubectl runner path. Say the fallback is source-controlled locally and name what is missing for GitLab automation.',
