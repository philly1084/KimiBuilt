...
class GitLocalTool extends ToolBase {
  ...
  async runSaveAndPush(repoRoot, params, timeout, tracker) {
    ...
    const branch = this.sanitizeBranch(params.branch || await this.getCurrentBranch(repoRoot));
    ...
    const committed = await this.spawnGit(['commit', '-m', commitMessage], {
      cwd: repoRoot,
      timeout,
    });
  ...
  async buildArgs(action, repoRoot, params) {
    switch (action) {
      ...
      case 'commit':
        return ['commit', '-m', this.sanitizeCommitMessage(params.message)];
      case 'push': {
        const remote = this.sanitizeRemote(params.remote || 'origin');
        const branch = this.sanitizeBranch(params.branch || await this.getCurrentBranch(repoRoot));
        const args = ['push'];
