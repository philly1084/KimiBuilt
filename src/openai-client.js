...
function buildAutomaticToolGuidance(automaticTools = [], options = {}) {
    ...
    if (automaticTools.some((entry) => entry.id === 'podcast')) {
        ...
        guidance.push('- For video podcast, podcast video, or MP4 podcast requests, call `podcast` with `includeVideo: true` and use the default waveform-card MP4 unless the user explicitly asks for scene imagery. For visual podcast, scene image, image-slide, infographic, or cover-art requests, set `videoRenderMode: "storyboard"`, `videoImageMode: "mixed"`, and `videoGenerateImages: true` unless the user explicitly asks not to generate images. Favor infographic slide variety: hook card, timeline, comparison board, process flow, risk/impact map, evidence dashboard, myth-vs-fact panel, and takeaway card.');
